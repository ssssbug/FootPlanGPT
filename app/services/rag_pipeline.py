...
# Internal imports
from utils.milvus_store import MilvusConnectionManager
from model.embedder import get_text_embedder, get_dimension
from llm.select_llm import LLM

# --- User's Original Helper Functions ---

def _get_markitdown_instance():
    """获得已配置 MarkItDown 实例，以进行文档转换"""
    try:
        from markitdown import MarkItDown
        return MarkItDown()
    except ImportError:
        logger.warning("MarkItDown is not installed. Please install it first.")
        return None

def _is_markitdown_supported_format(path:str)->bool:
    """
    检查MarkItDown是否支持该格式.支持的文件格式:PDF、Office文档(docx、xlsx、pptx)、图像(jpg、png、gif、bmp、tiff)、音频(mp3、wav、m4a)、HTML、文本格式(txt、md、csv、json、xml)、zip文件等
    """
...
def _convert_to_markdown(path:str)->str:
    """
    一款通用文档阅读器，采用MarkItDown技术，并增强了PDF处理能力，可将任何支持的文件格式转换
    为MarkItDown文本
    """
    ...
    #其他格式使用原有MarkItDown处理
    mid_markitdown = _get_markitdown_instance()
    if mid_markitdown is None:
        return _fallback_text_reader(path)
    
    try:
        result = mid_markitdown.convert(path)
...
def _post_process_pdf_text(text:str)->str:
    ...
    for line in lines:
        ...
        if re.match(r'^\d+$',line):#纯数字行(页码)
            continue
        if line.lower() in ['github','project','forks','starts','language']:
            continue
...
def _fallback_text_reader(path:str)->str:
    """
    当MarkItDown不可用时，一个简单的备用文本文件读取器
...
def _detect_lang(sample:str)->str:
    try:
        ...
        return detect(sample[:1000]) if sample else "unkown"
    except Exception:
        return "unkown"
...
def _is_cjk(ch:str)->bool:
    ...
    return (
        ...
        0x2F800 <= code <= 0x2FA1F #CJK扩展G
    )

def _approx_token_len(text:str)->int:
    #近似估计:CJK字符按1token，其他按空白分词
...
def load_and_chunk_texts(paths:List[str],chunk_size:int=800,chunk_overlap:int=100,namespace:Optional[str]=None,source_label:str = "rag")->List[Dict]:
    """
    一款基于MarkItDown的通用文档加载器和分块器，它能将所有支持的格式转换为Markdown，然后智能地进行分块
    """

    print(f"[RAG] 通用加载器开始：file={len(paths)} chunk_size = {chunk_size} overlap={chunk_overlap},ns={namespace or 'default'}")
    chunks:List[Dict]=[]
    seen_hashes = set()
    ...
    print(f"[RAG]通用加载器完成:total_chunks={len(chunks)}")
    return chunks
...
def _create_default_vector_store(dimension: int = None) -> Any:
    """
    Create default Milvus vector store with RAG-optimized settings.
    使用连接管理器避免重复连接。
    """
    if dimension is None:
        dimension = get_dimension(384)
    
    # 使用连接管理器 (Ensure import exists)
    from utils.milvus_store import MilvusConnectionManager
    return MilvusConnectionManager.get_instance(
...
def index_chunks(
    ...
    print(f"[RAG] Milvus upsert start: n={len(vecs)}")
    # Milvus expects: vectors, ids, texts, metadatas
    try:
        success = store.add_vectors(vectors=vecs, ids=ids, texts=texts, metadatas=metas)
        if success:
            print(f"[RAG] Milvus upsert done: {len(vecs)} vectors indexed")
        else:
            print(f"[RAG] Milvus upsert failed")
            raise RuntimeError("Failed to index vectors to Milvus")
    except Exception as e:
        print(f"[RAG] Milvus upsert error: {e}")
        raise e
...
from model.embedder import get_text_embedder, get_dimension
from llm.select_llm import LLM
from ..memory.storage.neo4j_store import Neo4jConnectionManager
...
def create_rag_pipeline(
    qdrant_url: Optional[str] = None, # Deprecated, kept for compat or can be removed
    collection_name: str = "hello_agents_rag_vectors",
    rag_namespace: str = "default",
    neo4j_enabled: bool = True
) -> Dict[str, Any]:
    """
    Create a complete RAG pipeline with Milvus, Neo4j, and unified embedding.
    ...
    neo4j_store = None
    if neo4j_enabled:
        neo4j_store = Neo4jConnectionManager.get_instance()
        if neo4j_store:
            print("[RAG] Neo4j Graph Store connected.")
        else:
            print("[RAG] Neo4j connection failed, GraphRAG disabled.")
    
    def add_documents(file_paths: List[str], chunk_size: int = 800, chunk_overlap: int = 100):
        """Add documents to RAG pipeline (Vector + Graph)"""
        ...
        # 2. Build Graph (if enabled)
        if neo4j_store and chunks:
            print(f"[RAG] Building graph for {len(chunks)} chunks...")
            build_graph_from_chunks(neo4j_store, chunks)
    ...
    def search_advanced(
        ...
        # Todo: Future enhancement - use neo4j_store to expand context via graph
    ...
    def get_stats():
        ...
        stats = text_store_stats = store.get_collection_stats()
        if neo4j_store:
            try:
