...
统一嵌入模块(实现+提供器)
说明：
 - 提供统一的文本嵌入接口与实现，本地Transformer、Dashscope、TF-IDF兜底
 - 暴露get_text_embedder()/get_dimension()/refresh_embedder()供各记忆类型统一使用
 - 通过环境变量优先级：dashscope>local>tfidf.

 环境变量：
  - EMBED_MODEL_TYPE：“dashscope”|"local"|"tfidf(默认local)
  - EMBED_MODEL_NANE：模型名称（local默认：jinaai/jina-embeddings-v3）
...
class  LocalTransformerEmbedding(EmbeddingModel):
  ...
  def _locd_backend(self):
    ...
    try:
      ...
      self._st_model = None
    raise ImportError(
      "未找到可用的本地嵌入后端，请安装sentence-transformers或transformers+torch"
    )
  ...
  def encode(self,texts:Union[str,List[str]]):
    ...
    if self._backend == "st":
      ...
      with torch.no_grad():
        outputs = self._hf_model(**tokenizer)
        embedding = outputs.last_hidden_state.mean(dim=1).cpu().numpy()

      vecs = [v for v in embedding]
...
class TFIDFEmbedding(EmbeddingModel):
  ...
  def _init_vectorizer(self):
    try:
      ...
      raise ImportError("未找到可用的TF-IDF后端，请安装scikit-learn")
...
class DashscopeEmbedding(EmbeddingModel):
  """
  阿里云Dashscope Embedding/OpenAI兼容REST模式
  行为：
    提供base_url 则使用OpenAI兼容的REST接口(POST{base_url}/embedding)
  否则使用官方dashscope SDK TextEmbedding.call
  """

  def __init__(self,model_name = "text_embedding-v3",api_key:Optional[str] = None,base_url:Optional[str] = None):
  ...
  def encode(self,texts:Union[str,List[str]]):
    ...
    if self.base_url:
      import requests
      url = self.base_url.rstrip("/") + "/embedding"
      headers = {
      ...
      if resp.status_code >=400:
        raise RuntimeError(f"Embedding RESR 调用失败:{resp.status_code}{resp.text}")
      data = resp.json()
      #期望结构:"{"data":[{"embedding":[...]}]}",
      items = data.get("data") or []

      vecs = [np.array(item.get("embedding") or []) for item in items]
      if single:
        return vecs[0]
      return vecs
    ...
    rsp = TextEmbedding.call(model=self.model_name,texts=inputs)
    embeddings_obj = None 
    if isinstance(rsp,dict):
      embeddings_obj = (rsp.get("output") or {}).get("embeddings")
    else:
      embeddings_obj = getattr(getattr(rsp,"output",None),
      "embedding",None)
...
def create_embedding_model(model_type:str="local",**kwargs):
  """创建嵌入模型实例
  model_type: "dashscope"|"local"|"tf-idf"
...
def create_embedding_model_with_fallback(model_type:str="local",**kwargs):
  ...
  if preferred_type in ("local","huggingface"):
    preferred_type = "local"
  
  fallback = ["local","dashscope","tf-idf"]
...
def _build_embedder()->EmbeddingModel:
  preferred = os.getenv("EMBEDDING_MODEL","local").strip()
  #根据提供商选择默认模型
  default_model = "jinaai/jina-embeddings-v3" if preferred == "local" else "sentence-transformers/all-MiniLM-L6-v2"
